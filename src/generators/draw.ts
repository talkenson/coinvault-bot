...
export const drawHorses = (series: number[][]) => {
  ...
  for (let frameId = 0; frameId < maxFrames; frameId++) {
    ...
    for (let horseId = 0; horseId < horses.length; horseId++) {
      ...
      ctx.fillStyle = `rgb(255,255,255)`;
      ctx.fillText(`${horseId + 1}`, x - 4, y + 5);
      ...
      if (place) {
        ...
        ctx.fillText(place.toFixed(0), WIDTH - START_X - 12, y + 5);
      }
    }
...
if (import.meta.main) {
  ...
  await Bun.write(Bun.file(`last.png`), buffers[buffers.length - 1], {
    createPath: true,
  });
  const gifBuffer = await renderFramesToGIF(buffers, { height, width });
  await Bun.write(Bun.file("file.gif"), gifBuffer, { createPath: true });
}
